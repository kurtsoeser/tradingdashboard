import { BookMarked, Route, Sparkles } from "lucide-react";
import { PageHeader } from "../PageHeader";
import { t } from "../../app/i18n";
import type { AppSettings } from "../../app/settings";

interface AiAssistantPlanViewProps {
  language: AppSettings["language"];
  onBackToAssistant: () => void;
  onOpenJournal: () => void;
}

export function AiAssistantPlanView({ language, onBackToAssistant, onOpenJournal }: AiAssistantPlanViewProps) {
  const useItems = [
  ...
  return (
    <section className="section ai-assistant-page ai-assistant-plan-page">
      <PageHeader
        title={
          <>
            <Route size={18} />
            {t(language, "aiRoadmapPageTitle")}
          </>
        }
      ...
      <div className="ai-assistant-plan-toolbar">
        <button type="button" className="secondary" onClick={onBackToAssistant}>
          <Sparkles size={16} aria-hidden />
          {t(language, "aiRoadmapBackToAssistant")}
        </button>
      </div>
